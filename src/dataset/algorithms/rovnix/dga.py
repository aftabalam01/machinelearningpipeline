...
usdeclar = open(f"{dir_path}/usdeclar.txt", 'r').read().strip().split()
for i in list(range(0, len(usdeclar))):
...
def generateSeed(a1, a2, a3):
    ...
    v6 = 0
    v7 = 0
    v8 = "1F1C1F1E1F1E1F1F1E1F1E1F".encode()
    v8 = binascii.hexlify(v8).decode()
    result = 0
    if (a1 > 0):
        if ((a2 - 1) <= 0xB):
            if ((a3 - 1) <= 0x1E):
                ...
                if (v4):
                    v8 = v8[:10] + str(int("0x1D", 16)) + v8[12:]
                v5 = 0
                if (a2 > 1):
                    v7 = v8
                    v6 = a2 - 1
