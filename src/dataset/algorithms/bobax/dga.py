...
def dga(index, date, seed_set, temp_file=None, tld_set_nr=1):
    ...
    discards = int(date.timestamp()) // 604800  + 2
    if domain_nr % 9 < 8:
        if domain_nr % 9 >= 6:
            discards -= 1
...
def get_domains(nr, date=datetime.now(), seed=random.choice(['a','b']), tld_set=1):
    domains = []
    for i in range(nr):
