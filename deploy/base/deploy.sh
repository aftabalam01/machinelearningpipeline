...
then
 docker_cmd=`echo "docker run -it --rm -v $curr_dir:/apps/ -v ~/.aws:/root/.aws -e AWS_DEFAULT_PROFILE=saml -w /apps aftabalam01/terragrunt:0.19.25 $TFCMD -auto-approve"`
else
 docker_cmd=`echo "docker run -it --rm -v $curr_dir:/apps/ -v ~/.aws:/root/.aws -e AWS_DEFAULT_PROFILE=saml -w /apps/ aftabalam01/terragrunt:0.19.25 $TFCMD"`
fi
